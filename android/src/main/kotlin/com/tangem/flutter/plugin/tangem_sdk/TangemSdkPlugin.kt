...
public class TangemSdkPlugin : FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    private fun createWallet(call: MethodCall, result: Result) {
        try {
            sdk.createWallet(null, cid(call), message(call)) { handleResult(result, it) }
        } catch (ex: Exception) {
            handleException(result, ex)
        }
    }
...
//            sdk.purgeWallet(null, cid(call), message(call)) { handleResult(result, it) }
    ...
    companion object {
        ...
        fun walletPublicKey(call: MethodCall): ByteArray {
            if (!call.hasArgument("walletPublicKey")) throw NoSuchFieldException("walletPublicKey")

            val arg = call.argument<String>("walletPublicKey")!!
...
data class SignResponse(val signatures: List<HexString>)